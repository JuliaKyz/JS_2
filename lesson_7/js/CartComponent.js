    ...
    template: `
        <div class="cart-item">
            <div class="product-bio">
                <img class="cart-item-img" :src="cart_item.imgPath" alt="Some img">
            ...
            <div class = "right-block">
                ...
                <button class="del-btn" @click="$parent.remove(cart_item)"> &times; 
                </button>
            </div>
        </div>`
    ...
    methods: {
        addProduct(product) {
            this.$parent.getJson(`${API}/addToBasket.json`)
                .then(data => {
                    if (data.result) {
                        let find = this.cartItems.find(el => el.id_product ===
                            product.id_product);
                        if (find) {
                            find.quantity++;
                        } else {
                            let prod = Object.assign({ quantity: 1 }, product) //создание нового объекта
